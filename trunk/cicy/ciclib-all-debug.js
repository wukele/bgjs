...
/**
 * @name CC.Ajax#caller
 * @property {Function} [caller] 用于调用onfailure,onsuccess函数的this对象.
 */
 
/**
 * @name CC.Ajax#onfailure
 * @property {Function} [onfailure]失败后的回调.
 */
 ...
 */
 /**
  *@property {Function} [onsuccess] 设置成功后的回调,默认为运行服务器返回的数据内容.
  */
    onsuccess: (function(ajax) {
        ajax.invokeHtml();
    }),
 ...
 */
  ...
  /**
 * 获得相对控件方位坐标.
 * @param {Base|DOMElement} tar 目标元素
 * @param {String} how 相对位置,可选值有c,l,r,t,b组合,如cc,lt,rb
 */
  getAnchorPos: function(tar, how) {
    how = how || 'cb';
    tar = CC.fly(tar);
    var txy = tar.absoluteXY(),
    tx = txy[0],
    ty = txy[1],
    sz = this.getSize(true),
    vh = CC.getViewHeight(),
    vw = CC.getViewWidth(),
    th = tar.getHeight(true),
    tw = tar.getWidth(true),
    nx,
    ny,
    fix = false;
...
/**
 * 连接服务器, onsuccess操作如果未在配置中指定,默认调用当前ConnectionProvider类的defaultLoadSuccess方法
 ...
 */
    ...
    if (!cfg.onsuccess)
    	cfg.onsuccess = this.defaultLoadSuccess;
/**
 ...
 */
	active : function(expand){
		var c = this.pCt;
		if(!this.disabled && c.onItem !== this){
			c.onItem = this;
...
/**@private*/
    ...
    if(!this._onCtxClick) {
	    var onclick = (function() {
	      ...
	      oThis.contexted = 0;
	      Event.un(document, 'click', onclick);
	    }
	    );
	    
	    this._onCtxClick = onclick;
  	}
    Event.on(document, 'click', this._onCtxClick);
    this.contexted = 1;
	},
  ...
  showMenu: function(a, b) {
	    ...
	    if (typeof a !== 'number') {
	      ...
	      this.setXY(this.getAnchorPos(f, 'cb'));
	      f.unfly();
	    } else {
	      this.setXY(a, b);
	    }
  ...
  _autoPositioned : function() {
    ...
    if (pi) {
    	if(pi.menubar){
      	this.setXY(this.getAnchorPos(pi, 'cb'));
      	return;
      }
      var pos = this.getAnchorPos(pi,'rc');
      if(!pos[2])
      	pos[0]-=2;
      pos[1]+=2;
      this.setXY(pos);
   ...
   */
    ...
    showDatepicker: function(b) {
      ...
      if (b) {
        ...
        //get the right position.
        //callback,cancel, caller, childId, cssTarget, cssName
        dp.setXY(dp.getAnchorPos(this, 'cb'))
          .bindContext(this.onDatepickerContexted, false, this, null, this, this.contextCS)
          .focus(0);
      }
    },
 ...
 */
    ...
    preferPosition: function() {
      ...
      s.setXY(s.getAnchorPos(this, 'cb'));
    },
